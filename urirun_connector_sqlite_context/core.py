...
DATA = _urirun_compat.connector(CONNECTOR_ID, scheme="data")
ARTIFACT = _urirun_compat.connector(CONNECTOR_ID, scheme="artifact")
CHECK = _urirun_compat.connector(CONNECTOR_ID, scheme="check")
LOG = _urirun_compat.connector(CONNECTOR_ID, scheme="log")
...
@DATA.handler("data://host/doctor/query/report", isolated=True, meta={"label": "Connector readiness report"})
...
def _connector_version() -> str:
    try:
        ...
        return version("urirun-connector-sqlite-context")
    except Exception:
        return "0.1.0"
...
def connector_manifest() -> dict[str, Any]:
    ...
    return DATA.manifest(_urirun_compat.load_manifest(__package__))


def main(argv: list[str] | None = None) -> int:
    """Console-script entry point: subcommands + dispatch derived from the handlers."""
    return DATA.cli(argv, manifest_prose=_urirun_compat.load_manifest(__package__))
